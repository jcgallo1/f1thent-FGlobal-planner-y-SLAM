...
class DStar(GraphSearcher):
    ...
    """
    def __init__(self, start: tuple, goal: tuple, env: Grid,
                 clearance_map=None, clearance_weight: float = 0.0) -> None:
    ...
    def cost(self, node1: DNode, node2: DNode) -> float:
        """Costo de movimiento con penalizacion opcional cerca de paredes."""
        base_cost = super().cost(node1, node2)
        if not math.isfinite(base_cost) or self.clearance_map is None:
            return base_cost

        clearance = min(
            float(self.clearance_map[node1.y, node1.x]),
            float(self.clearance_map[node2.y, node2.x]),
        )
        penalty = self.clearance_weight / max(clearance, 0.5) ** 2
    ...
    def plan(self) -> tuple:
        ...
        Returns:
            ...
            _ (None): None
        """
        while self.start.t != 'CLOSED':
            if not self.OPEN:
                raise RuntimeError(
                    "D* no encontro una ruta entre el inicio y la meta."
                )
            self.processState()
    ...
    def min_k(self) -> float:
        ...
        """
        node = self.min_state
        return node.k if node is not None else -1
    ...
    def getNeighbor(self, node: DNode) -> list:
        ...
        for motion in self.motions:
            # Un nodo situado en el borde puede producir un vecino fuera de
            # las dimensiones del mapa. Ignorarlo evita acceder a una clave
            # inexistente, por ejemplo (250, y) en un mapa de ancho 250.
            n = self.map.get((node + motion).current)
            if n is not None and not self.isCollision(node, n):
                neighbors.append(n)
        return neighbors
